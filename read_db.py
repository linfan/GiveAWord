...
def resize_all_images(src_folder, tar_folder, width):
    for file in os.listdir(src_folder):
        image = Image.open('{}/{}'.format(src_folder, file))
        size = image.size
        image = image.resize((width, int(float(width) * size[1] / size[0])),
                             Image.BILINEAR)
        image.save('{}/{}'.format(tar_folder, file))
...
#            copy_files('book/book_sentence', word, T_SENTENCEVIDEO)
    ...
    if create_folder('word/word_images'):
        resize_all_images('book/book_images', 'word/word_images', 150)
    if create_folder('word/word_deformation'):
        resize_all_images('book/book_deformation', 'word/word_deformation', 150)
...
def write_html_file():
    with open('book/book.html', mode='w', encoding='utf-8') as file:
        ...
        for word in word_list:
            ...
            file.write('<td align="center" valign="middle">')
            file.write('<img src="{}"></td>\n'.format(target_path))
            
            # second column
            file.write('<td align="left" valign="middle" width="500"><font face="Microsoft Yahei" size="4"><b>{}</b></font> <font size="3">{} {}'.format(
                word[T_WORD], word[T_PHONETIC], word[T_WORDMEAN]))
#            if word[T_WORDMEAN_EN]:
#                file.write('<br><英译> {}'.format(word[T_WORDMEAN_EN]))

            file.write('<br><font face="Microsoft Yahei">[例]</font> {} {}'
                    .format('<u><b>{}</b></u>'.format(word[T_WORD])
                    .join(re.split(word[T_WORD], word[T_SENTENCE], flags = re.IGNORECASE)), word[T_SENTENCETRANS]))

            if word[T_ETYMA]:
                file.write('<br><font face="Microsoft Yahei">[词源]</font> {}'.format(word[T_ETYMA]))
                
            have_variant = False
            for variant in word[T_WORDVARIANTS]:
                if variant != word[T_WORD]:
                    if not have_variant:
                        file.write('<br><font face="Microsoft Yahei">[变式]</font> {}'.format(variant))
                        have_variant = True
                    else:
            ...
            # third column
            file.write('<td align="left" valign="middle">')
            if word[T_DEFORMATION_IMG]:
                target_path = './book_deformation/{}.{}'.format(word[T_WORD], 
                        word[T_DEFORMATION_IMG].rsplit('.', 1)[1]).replace(' ', '_');
                file.write('<img src="{}">'.format(target_path))
                if word[T_DEFORMATION_DESC]:
                    file.write('</p><font size="3">{}</font>'.format(word[T_DEFORMATION_DESC]))
            file.write('</td>\n')
            file.write('</tr>\n')

            # second row
            file.write('<tr><td colspan=3><font size="1"><hr size=1></font></td></tr>\n')
            
        file.write('</table></body></html>')

def create_word_file():
    book = open('book/book.html', 'r', encoding='utf-8')
    word = open('word/word.html', 'w', encoding='utf-8')
    for line in book.readlines():
        word.write(line.replace('/book_', '/word_'))
    book.close()
    word.close()
